...
function WeightVis(props: any): any {
  ...
  const currentPath = d3.select(ref.current).select(".bar-plot-area");

  const handleChange = useCallback(
    (i: number, d: any) => {
      // k有効数字3けたまでで一致する
      if (data[i] && d.toFixed(3) != data[i].toFixed(3)) {
        handleWeightChange([...data.slice(0, i), d, ...data.slice(i + 1)]);
      }
    },
  ...
  useEffect(() => {
    ...
    getMergedPath(currentPath, "g", "axisBottom")
      ...
      .call(d3.axisBottom(xScale).tickFormat((d: any) => labels[d - 0.5]))
  ...
  return (
    <svg ref={ref} className="weight-chart" preserveAspectRatio="xMidYMid meet">
